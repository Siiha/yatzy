...
class player:
	...
	def potential(self):
		...
		self.options.update({
			...
			'two pairs':max([i*c[i]+j*c[j] if c[i]>=2<=c[j] else 0 for i in range(1,7) for j in range(1,7) if i != j]),
			'triple number':max([i*c[i] if c[i]==3 else 0 for i in range(1,7)]),
			...
			'full house': sum(self.row) if len(c)==2 and set((2,3))==set(c.values()) else 0,
			'random': sum(self.row),
...
while t < 15:
	...
	for i in range(2):
		...
		if x: 
			...
			p.change([int(i)-1 for i in input("Enter the rerolled dice 1-5 separated by a space: ").split()])
			print(p.row)
	for i in p.minutes:
		print(i+":"+str(p.minutes[i]))
	p.mark(input("Which point in minutes do you want to fill in? You can only select those whose value is None.: "))
